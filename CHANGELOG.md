...
### Added
- `src/generate.py` — módulo completo de geração: `generate(query, hits, client) → GenerationResult`
  - System prompt com `cache_control="ephemeral"` (prompt caching ativo)
  - Citações inline `[N]` extraídas por regex; `citations[]` estruturado no output
  - Anti-alucinação: responde APENAS pelo contexto; fallback sem invenção
  - Streaming por padrão no CLI; `--no-stream` / `--json` para batch/Ragas
  - CLI completo: `--query`, `--top-k`, `--tipo-ato`, `--year`, `--no-rerank`, `--device`, `--json`
- `Makefile` — target `generate` no Caminho 2: `make generate QUERY="..."` (carrega `.env`)

### Changed
- `README.md` — Fase 6 expandida de "📋 planejada" para "✅ concluída" com schema, pipeline, decisões e comandos
- `HANDOFF.md` — Fase 6 movida para "Fases concluídas"; Fase 6 removida de "Próximas fases"

### Notes
- Smoke end-to-end com API real requer `.env` com `ANTHROPIC_API_KEY` preenchida
- Interface Python reutilizável diretamente pela Fase 7 (Ragas) sem overhead de CLI
...
### Added
- `src/retrieve.py` — módulo completo de retrieval híbrido:
  - `Hit` dataclass com `chunk_id`, `score`, `score_rrf`, `score_rerank`, `rank_dense`, `rank_bm25`, `payload`
  - `CrossEncoderReranker` — substituto interno do `FlagReranker` (quebrado no `transformers 5.6+`), usa `AutoTokenizer(use_fast=True)` + `AutoModelForSequenceClassification` diretamente; API compatível (`compute_score(pairs, normalize=True) -> list[float]`)
  - `detect_device()` — autodetect CUDA → CPU (MPS pulado por padrão; flag `--device` permite forçar)
  - `load_bm25(path)`, `load_embedder(device)`, `load_reranker(device)` — loaders explícitos sem singleton/cache mágico
  - `_dense_search()` e `_bm25_search()` rodando em paralelo via `ThreadPoolExecutor(max_workers=2)`
  - `rrf_fuse(lists, k=60)` — RRF manual no cliente com 2 listas (dense + BM25); sparse bge-m3 fora (dense já cobre polissemia, BM25 cobre jargão raro com IDF do corpus)
  - `_apply_filter()` — filtros opcionais por `tipo_ato`, `year`, `tier` tanto no Qdrant (`query_filter`) quanto no BM25 (numpy mask)
  - `retrieve(query, bm25_data, qdrant_client, embedder, reranker, top_k, filters, parallel) -> list[Hit]`
  - CLI `python -m src.retrieve --query "..." --bm25 artifacts/bm25_index.pkl [--no-rerank] [--device cpu] [--top-k 10]`

### Notes
- Smoke validado (CPU, Apple M-series): query "TUSD" → top-1 REH 2022 Art. 13 (rerank=0.9976, era dense #4 antes do rerank). Latência 10.8 s dominada pelo reranker em CPU (~280 ms × 30 pares)
- `--no-rerank` flag disponível para retrieval rápido sem cross-encoder
- Nenhuma dependência nova adicionada ao `requirements.txt` (transformers/torch já estavam lá pelo bge-m3)
...
### Added
- `Makefile` (~140 linhas) com targets organizados em 3 categorias visíveis no `make help`:
  - **Caminho 2 (recomendado):** `restore-artifacts` (chain qdrant-up + download-artifacts + upload-snapshot), `smoke`
  - **Caminho 1 (do zero):** `download`, `analyze`, `parse`, `chunk`, `index`, `all`
  - **Infra & utilitários:** `qdrant-up`, `qdrant-down`, `clean-artifacts`, `clean-qdrant`, `clean-collection`
- Detecta OS (Windows → `.venv/Scripts/python.exe`, Linux/Mac → `.venv/bin/python`); todas as variáveis sobrescrevíveis na linha de comando (`make index BATCH_SIZE=64`, `make smoke PYTHON=python3`, etc.)
- `RELEASE_TAG ?= v0.4.0` parametrizado — quando subirmos v0.5.0+ basta atualizar uma linha
- `make` sem argumento → roda `help` (default goal)
- Targets de download usam regra com prereq de arquivo (`$(SNAPSHOT)`) — Make pula download se artefato já existe localmente, comportamento idempotente

### Changed
- `README.md`:
  - Pré-requisitos comuns: adiciona GNU Make + instruções de instalação por OS (apt, brew, choco/scoop)
  - Caminho 2: `make restore-artifacts && make smoke` como comando principal; equivalente raw bash mantido logo abaixo pra quem prefere
  - Caminho 1: `make all` ou fase-por-fase via `make download/parse/chunk/index`
  - Caminho 3: `make smoke` (substitui invocação direta do script)
  - Roadmap: Makefile vira ✅ concluída; Fase 5 atualizada pra "🔨 Em andamento (@pedro)"
- `HANDOFF.md`:
  - "Em execução agora" — Pedro dono da Fase 5; eu marco Makefile como concluído
  - Adicionada entrada em "Fases concluídas" com lista completa dos targets, variáveis customizáveis e prova de validação end-to-end (restore + smoke rodaram do zero)
  - Removida a entrada antiga "Makefile (livre, nice-to-have)" das próximas fases

### Validation
- `make help` mostra menu organizado por categoria com cores ANSI
- `make restore-artifacts` rodou end-to-end em 14s (artifacts já existiam localmente, então pulou o download de 1.46 GB; só fez upload + validate). Coleção `aneel_chunks` voltou green com 160.267 pontos.
- `make smoke` rodou após o restore: 5 queries dense retornaram top-3 idêntico à sessão anterior (REN 1000 Art 528 pra "prazo de ligação", REN 1000 Art 655-B pra "microgeração", NREH pra "TUSD"...).

### Notes
- Motivação: o Claude do Pedro tentou o Caminho 2 e relatou atrito pela ausência do Makefile. Subiu de "cosmético" pra "tem que fazer" — sinal real de uso > minha estimativa anterior.
